...
test("AgentAxis ana sayfasi guclu giris metniyle sunulur", async () => {
  ...
  assert.match(html, /Müşterilere geç mi dönüyorsunuz/);
  ...
  assert.match(html, /Hangi hizmete ihtiyacınız olduğunu bilmiyor olabilirsiniz/);
  assert.match(html, /10<\/b><span>GÜNLÜK/);
  assert.match(html, /Bu deneme her işletmeye bir kez sunulur/);
  assert.match(html, /Önce 10 gün ücretsiz deneyin/);
  ...
  assert.match(html, /Size sadece fikir vermeyiz/);
...
test("ana donusum ve guvenlik unsurlari korunur", async () => {
  ...
  assert.match(html, /Ücretsiz Google Haritalar incelemesi/);
